	...
	public function buscarPersonalizadaPagamentos($identificador, $listOrigem, $codOrigem, $listaOperadoras, $dataI, $dataF, $listaStatus, $listaFormaPgto, $valorTransacao) {
		...
		if ($listOrigem [0] !== "") {
			...
			for($i = 0; $i < (count ( $listOrigem ) - 1); $i ++) {
				if ($i == (count ( $listOrigem ) - 2))
					$whereOrigem .= "transacao.fk_origem = " . $listOrigem [$i] . "";
				else
					$whereOrigem .= "transacao.fk_origem = " . $listOrigem [$i] . " OR ";
			}
			$whereOrigem .= ")";
			$sql .= $whereOrigem;
		}
		...
		if ($codOrigem != "" && $codOrigem != 0){
			if ($previous)
				...
				$sql .= " WHERE ";
			$previous = true;
			$whereCodOrigem = "(transacao.codigo_origem = $codOrigem)";
			$sql .= $whereCodOrigem;
		}
		...
		if ($listaOperadoras [0] !== "") {
			...
			for($i = 0; $i < (count ( $listaOperadoras ) - 1); $i ++) {
				if ($i == (count ( $listaOperadoras ) - 2))
					$whereOperadora .= "operadora_empresa.id_operadora_empresa = " . $listaOperadoras [$i] . "";
				else
					$whereOperadora .= "operadora_empresa.id_operadora_empresa = " . $listaOperadoras [$i] . " OR ";
			}
			$whereOperadora .= ")";
			$sql .= $whereOperadora;
		}
