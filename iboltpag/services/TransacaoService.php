...
$this->zerosEsquerda ( ($_i + 1), 6 ); // NÚMERO SEQUENCIAL DO REGISTRO - TAMANHO 6 / 395-400 NUM
	...
	public function gerarRemessaBancodoBrasil400Dia($dataI, $dataF, $banco, $empresa) {
		...
		$this->gerarRemessaBancodoBrasil400 ( $lstTransacoes );
		array_push ( $retorno, 1 );
		array_push ( $retorno, $lstTransacoes [0] ["nome_arquivo"] );
		return $retorno;
	}
	public function gerarRemessaBancodoBrasil400($lstTransacoes) {
		...
		for($_i; $_i <= count ( $lstTransacoes ); $_i ++) {
			...
			$regTransacao .= 
				...
				$this->zerosEsquerda ( $lstTransacoes [($_i - 1)] ["fk_pedido_pagamento"], 10 ) . // IDENTIFICAÇÃO DO TÍTULO NO BANCO - TAMANHO 9 / 071-080 NUM (NOSSO NÚMERO)
				//FIM NOSSO NÚMERO
		...
		$reg9 = "5" . // IDENTIFICAÇÃO DO REGISTRO - TAMANHO 1 / 001-001 NUM DEFAULT '5'
			"03" . 	//TIPO DE SERVIÇO - TAMANHO 2 / 002-003 DEFAULT '03'
			$this->gerarEspacosBrancos ( 15 ) . // IDENTIFICAÇÃO DO TITULO - TAMANHO 15 / 004-018 BRANCOS
	...
	public function remontarRemessaBancodoBrasil400($lstTransacoes) {
		...
		$reg0 = "" . "0" . // IDENTIFICAÇÃO DO REGISTRO - TAMANHO 1 / 001-001 DEFAULT '0'
			...
			for($_i; $_i <= count ( $lstTransacoes ); $_i ++) {
				...
				$this->gerarEspacosBrancos ( 1 ) . // Indicativo de Mensagem ou Sacador/Avalista - TAMANHO 1 / 088-088 branco ( Poderá ser informada nas posições 352 a 391 qualquer mensagem para ser impressa no boleto.)
				$this->gerarEspacosBrancos ( 3 ) . // Indicativo de Mensagem ou Sacador/Avalista - TAMANHO 3 / 089-091 branco
				$this->zerosEsquerda ( $lstTransacoes [($_i - 1)] ["codigo_variacao_carteira_banco_brasil"], 3 ) . // Variação da carteira - tamanho 3 / 092-094
				$this->gerarEspacosZeros ( 1 ) . // conta caução - tamanho 1 / 095-095 "0"
				$this->gerarEspacosZeros ( 6 ) . // número borderô - tamanho 6 / 096-101
				$this->zerosEsquerda ( $lstTransacoes [($_i - 1)] ["tipo_cobranca_banco_brasil"], 5 ) . // Tipo de Cobrança - tamanho 5 / 102-106
				$this->zerosEsquerda ( $lstTransacoes [($_i - 1)] ["codigo_carteira"], 2 ) . // Carteira - tamanho 2 / 107-108
	...
	public function autenticarInsert($usr, $pwd){
		...
		$consulta->close ();
		if (count($lstEmpresa) != 1) return null;
	...
	public function getFormaPagamentoOperadoraEmpresa($codEmpresa, $formaPagamento, $codOp){
		...
		if ($totalF != 1) {
			if ($totalF > 1)
				return array('CodStatus' => 2, 'Msg' => "Há mais de uma operadora cadastrada para esta forma de pagamento. Corrija e tente novamente");
			else 
				return array('CodStatus' => 3, 'Msg' => "Nenhuma operadora cadastrada para esta forma de pagamento!");
		}else 
			return array('CodStatus' => 1, 'Msg' => "Success!", 'Model' => $lstFormaPagOpeEmp[0]);
	...
	public function insertTransaction($dados, $fkFormaPgOpEm){
		...
		try {
			...
			if ($this->banco->getConexaoBanco ()->query ( $sql )){
				...
				return array('CodStatus' => 1, 'Msg' => "Success!", 'Model' => $id);
			}else {
				$consulta->close ();
				return array('CodStatus' => 2, 'Msg' => "Falha ao inserir novo registro!");
			}
		} catch (Exception $e) {
	...
	public function getOperadorasPorEmpresa($formaP, $usr, $pwd){
		try {
			...
			if (count($lstEmpresa) != 1) return array('CodStatus' => 2, 'Msg' => 'Falha na autenticação da solicitação!');
			...
			$consulta->close ();
			return array('CodStatus' => 1, 'Msg' => 'Sucess', 'Model' => $lstOperadoras);
	...
	public function getTransacao($idTransacao, $usr, $pwd){
		try {
			...
			if (count($lstTransacao) != 1) return array('CodStatus' => 2, 'Msg' => 'Falha ao identificar o documento');
			else {
				$lstTransacao[0]["Empresa"] = $lstEmpresa[0];
				return array('CodStatus' => 1, 'Msg' => 'Sucess', 'Model' => $lstTransacao[0]);
			}
